...
void RPCTwinMuxLinkMapHandler::getNewObjects()
{
    ...
    RPCTwinMuxLinkMap * _twinmux_link_map_object = new RPCTwinMuxLinkMap();
    RPCTwinMuxLinkMap::map_type & _twinmux_link_map
    ...
    while (_input_file) {
        ...
        edm::LogInfo("RPCTwinMuxLinkMapHandler") << "wheel " << _wheel << ", sector " << _sector;
        ...
        _twinmux_link_map.insert(std::pair<RPCTwinMuxLink, RPCLBLink>(RPCTwinMuxLink(_fed, _amc_number, _tm_input)
                                                                      , _lb_link));
    ...
    if (!txt_file_.empty()) {
        ...
        for (RPCTwinMuxLinkMap::map_type::const_iterator _link = _twinmux_link_map.begin()
