...
public:
    ...
    static unsigned int const npins_           = 16;
...
public:
    ...
    RPCFebConnector & setPins(::uint16_t _pins);

    bool isActive(unsigned int _pin) const;
    unsigned int getNPins() const;
    unsigned int getStrip(unsigned int _pin) const;

    bool hasStrip(unsigned int _strip) const;
    unsigned int getPin(unsigned int _strip) const;
